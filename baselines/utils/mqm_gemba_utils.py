        ...
        for line in x.split('\n'):
            ...
            if "critical" in line or "major" in line or "minor" in line:
                if not any([line.startswith(x) for x in
                            ['accuracy', 'fluency', 'locale convention', 'style', 'terminology', 'non-translation',
                             'other']]):
                    print(line)
...
def apply_template_shared_task_like_mt(source_lang, target_lang, source_seg, target_seg):
    tmp = apply_template(TEMPLATE_GEMBA_MQM,
                         {"source_lang": source_lang, "target_lang": target_seg, "source_seg": source_seg,
                          "target_seg": target_seg})
    string = "(system) " + tmp[0]["content"] + "\n\n"
    string += "(user) " + tmp[1]["content"] + "\n\n"
    string += "(assistant) " + tmp[2]["content"] + "\n"
    string += "(user) " + tmp[3]["content"] + "\n\n"
    string += "(assistant) " + tmp[4]["content"] + "\n"
    string += "(user) " + tmp[5]["content"] + "\n\n"
    string += "(assistant) " + tmp[6]["content"] + "\n"
    string += "(user) " + tmp[7]["content"] + "\n\n"
    string += "(assistant) "
    return string


if __name__ == "__main__":
    string = apply_template_shared_task_like_mt("English", "German", "hello", "hallo")
    print(string)
